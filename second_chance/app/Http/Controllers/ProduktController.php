    ...
    public function show($id)
    {
        ...
        $podobneProdukty = Produkt::where('id', '!=', $produkt->id)
            ...
            ->where(function($q) use ($produkt) {
                ...
                // farba je teraz JSON pole, porovnávame prienik
                foreach (($produkt->farba ?? []) as $farba) {
                    $q->orWhereJsonContains('farba', $farba);
                }
            })
            ->with('hlavnyObrazok')
            ...
            ->map(function($p) use ($produkt) {
                ...
                // počet zhodných farieb
                $zhodne = array_intersect($p->farba ?? [], $produkt->farba ?? []);
                if (count($zhodne) > 0) $matches += 2;
                $p->matches = $matches;
                return $p;
            })
            ->sortByDesc('matches')
    ...
    public function hladat(Request $request)
    {
        ...
        $query = Produkt::with('hlavnyObrazok')
            ...
            ->where(function($q) use ($search) {
                $q->where('nazov', 'like', '%' . $search . '%')
                  ->orWhere('znacka', 'like', '%' . $search . '%')
                  ->orWhere('popis', 'like', '%' . $search . '%')
                  ->orWhereJsonContains('farba', $search);
            });

        // rovnaké filtre ako v index()
        if ($request->filled('min_cena')) {
            $query->where('cena', '>=', $request->min_cena);
        }
        if ($request->filled('max_cena')) {
